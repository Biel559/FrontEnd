...
router.get('/reservations', authMiddleware, async (req, res) => {
  try {
    const user = await User.findById(req.user.id).populate('reservations.bookId', 'title author');
...
router.put('/:id/decrease-quantity', async (req, res) => {
  ...
  try {
    ...
    book.quantity -= decreaseBy; // Reduz a quantidade
    await book.save();
...
router.put('/:id/increase-quantity', async (req, res) => {
  ...
  try {
    ...
    book.quantity += increaseBy; // Aumenta a quantidade
    await book.save();
